...
class OrderDetailView(LRM, View):
    ...
    def get(self, response, pk):
        ...
        if cur_hour < 6 or cur_hour >= 18:
            ...
            if (
                diff.days == 0 and diff.seconds <= 43200 or priv == True
            ):  # users can delete orders upto 12 hours after placing them...
...
class OrderCreateView(LRM, View):
    ...
    def get(self, response):
        ...
        if cur_hour >= 6 and cur_hour < 18:
            ctx[
                "message"
            ] = "Order can only be placed between 6:00 pm to 6:00 am the next day"
    ...
    def post(self, response):
        ...
        if cur_hour >= 6 and cur_hour < 18:
            ctx = dict()
            ctx["privilege"] = get_object_or_404(
                User_Type, user=response.user
            ).privilege
            ctx[
                "message"
            ] = "Order can only be placed between 6:00 pm to 6:00 am the next day"
        ...
        try:
            ...
            order.delete()
            return HttpResponse(
                "Unable to process your order. Please go back to the previous page and order again"
            )
...
class OrderDeleteView(LRM, View):
    ...
    def get(self, response, pk=None):
        ...
        if priv == True:  # the admin can delete any order at any time
            return render(
                response, self.template_name, {"order": order, "privilege": priv}
            )
        else:
            if (
                order.user == response.user
            ):  # extra check to make sure that current user owns the order
                cur_time = dt.datetime.now()
                cur_hour = int(cur_time.strftime("%H"))
                ...
                if cur_hour >= 6 and cur_hour < 18:
                    ...
                    ctx[
                        "message"
                    ] = "Order can only be deleted between 6:00 pm to 6:00 am the next day"
                    return render(response, self.wrong_time_page, ctx)
                else:
                    created_time = order.created_at
                    diff = cur_time - created_time
                    if (
                        diff.days == 0 and diff.seconds <= 43200
                    ):  # users can delete orders upto 12 hours after placing them...
                        return render(
                            response,
                            self.template_name,
                            {"order": order, "privilege": priv},
                        )
                    else:
                        ctx = dict()
                        ctx["privilege"] = False
                        ctx[
                            "message"
                        ] = "Order cannot be deleted 12+ hours after it has been placed"
                        return render(response, self.wrong_time_page, ctx)
            else:
                return HttpResponse(
...
class OrderUpdatePaymentView(LRM, View):
    ...
    def get(self, response, pk=None):
        ...
        if (
            priv == False
        ):  # This scenario might arise if a non-admin user lands on the url by mistake. I am simply redirecting them to the /views/ page
            return redirect(self.success_url)
        else:
